...
from ..models import Product, Order, OrderSummary, User

class EcomModelsTestCase(TestCase):
    def setUp(self):
        user = User.objects.create(username='test', password='Viktorek1', email='example@gmail.com')
        product = Product.objects.create(name='T-shirt', description='A cotton t-shirt',
                               price=59.99, stored_quantity=10000)
        order_summary = OrderSummary.objects.create(client=user, total_price=0, address='Address', city='City', phone_number='123456789')
